...
import org.ghostprotocol.chat.device.DevicesGrpc;
...
class DevicesGrpcServiceTest extends SimpleBaseGrpcTest<DevicesGrpcService, DevicesGrpc.DevicesBlockingStub> {
  ...
  void setCapabilities(
    ...
    if (storage) {
      requestBuilder.addCapabilities(org.ghostprotocol.chat.common.DeviceCapability.DEVICE_CAPABILITY_STORAGE);
    }

    if (transfer) {
      requestBuilder.addCapabilities(org.ghostprotocol.chat.common.DeviceCapability.DEVICE_CAPABILITY_TRANSFER);
    }

    if (deleteSync) {
      requestBuilder.addCapabilities(org.ghostprotocol.chat.common.DeviceCapability.DEVICE_CAPABILITY_DELETE_SYNC);
    }

    if (versionedExpirationTimer) {
      requestBuilder.addCapabilities(org.ghostprotocol.chat.common.DeviceCapability.DEVICE_CAPABILITY_VERSIONED_EXPIRATION_TIMER);
    }
