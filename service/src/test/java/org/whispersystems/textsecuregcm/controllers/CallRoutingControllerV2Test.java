...
class CallRoutingControllerV2Test {
  ...
  void testGetRelaysSignalRoutingOnly() {
    ...
    try (Response rawResponse = resources.getJerseyTest()
      ...
      final TurnToken firstRelay = relays.get(0);
      assertThat(firstRelay.username()).isNotEmpty();
      assertThat(firstRelay.password()).isNotEmpty();
      assertThat(firstRelay.hostname()).isEqualTo(options.hostname());
      assertThat(firstRelay.urlsWithIps()).isEqualTo(options.urlsWithIps().get());
      assertThat(firstRelay.urls()).isEqualTo(options.urlsWithHostname().get());
    }
  }
  ...
  void testGetRelaysBothRouting() {
    ...
    try (Response rawResponse = resources.getJerseyTest()
      ...
      assertThat(relays.get(0)).isEqualTo(CLOUDFLARE_TURN_TOKEN);
  ...
  void testGetRelaysInvalidIpSuccess() throws UnknownHostException {
    ...
    try (Response rawResponse = resources.getJerseyTest()
      ...
      TurnToken token = response.relays().get(0);
      assertThat(token.username()).isNotEmpty();
      assertThat(token.password()).isNotEmpty();
      assertThat(token.hostname()).isEqualTo(options.hostname());
      assertThat(token.urlsWithIps()).isEqualTo(options.urlsWithIps().get());
