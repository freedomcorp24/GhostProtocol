...
import static org.ghostprotocol.util.AttributeValues.b;
import static org.ghostprotocol.util.AttributeValues.n;
import static org.ghostprotocol.util.AttributeValues.s;
...
import org.ghostprotocol.subscriptions.ProcessorCustomer;
  ...
  public CompletableFuture<byte[]> getSubscriberUserByProcessorCustomer(ProcessorCustomer processorCustomer) {
    ...
    return client.query(query).thenApply(queryResponse -> {
      ...
      if (count == 0) {
        ...
        Map<String, AttributeValue> result = queryResponse.items().stream().findFirst()
            .orElseThrow(() -> new IllegalStateException("Expected at least one item in query response"));
