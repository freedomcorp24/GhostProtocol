...
SIGNUP_RESPONSE=$(curl -s -X POST -H "Content-Type: application/json" -d '{"username":"testuser4","password":"testpassword"}' $LOCAL_API_URL/api/auth/signup)
echo $SIGNUP_RESPONSE

# Extract the token from the signup response
TOKEN=$(echo $SIGNUP_RESPONSE | ../extract_token.py)
echo "Extracted token: $TOKEN"

# Test the login endpoint
echo -e "\n\nTesting login endpoint..."
LOGIN_RESPONSE=$(curl -s -X POST -H "Content-Type: application/json" -d '{"username":"testuser4","password":"testpassword"}' $LOCAL_API_URL/api/auth/login)
echo $LOGIN_RESPONSE

# Extract the token from the login response
TOKEN=$(echo $LOGIN_RESPONSE | ../extract_token.py)
echo "Extracted token: $TOKEN"
...
ADMIN_TOKEN=$(echo $ADMIN_LOGIN_RESPONSE | ../extract_token.py)
echo "Extracted admin token: $ADMIN_TOKEN"
